#!/usr/bin/env python3
"""LOOM Stretch 020 helper — M5 with twenty-four-layer persistent hotset.

Reuses the frozen Stretch 017 M=5 exact oracle path and changes one runtime
factor only: persistent transformer hotset layers 0..7 -> 0..23. All model,
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"H24 transform failed for {label}: expected 1 occurrence, found {count}")
...
def transformed_013_wrapper(source013: str, stretch017) -> str:
    ...
    # checked by the balanced Stretch 020 parent using hotset provenance.
    transformed = transformed.replace(
        "LOOM Stretch 017 — Five-Token Oracle Block Confirmation",
        "LOOM Stretch 020 — Five-Token H24 Hotset Variant",
    )
    transformed = transformed.replace(
        "Stretch 017 — Five-Token Oracle Block Confirmation",
        "Stretch 020 — Five-Token H24 Hotset Variant",
    )
    transformed = transformed.replace(
        "five-token-oracle-block-confirmation-017",
        "five-token-h24-hotset-020-h24",
    )
    ...
    transformed = replace_once(
        ...
        '        "HOTSET_LAYER_IDS = tuple(range(24))",\n',
        "required H24 fragment",
    )
    ...
    # only because it is the derived residency/provenance gate for H24.
    ...
    replacement = (
        ...
        "        \"HOTSET_LAYER_IDS = tuple(range(24))\\nEXPECTED_HOTSET_BYTES = 24 * EXPECTED_LAYER_BYTES\\n\",\n"
        "        1,\n"
        "        \"twenty-four-layer persistent hotset\",\n"
        "    )\n"
        "    source = add_oracle_block_transform(source)\n"
    )
    transformed = replace_once(transformed, anchor, replacement, "H24 injection")

    transformed = transformed.replace(
        'print("Eight-layer persistent hotset: PRESERVED")',
        'print("Persistent hotset: H24 layers 0..23 (only changed runtime factor)")',
    )
    return transformed
...
def main() -> int:
    ...
    if not is_child:
        print("LOOM Stretch 020 — Five-Token H24 Hotset Variant frozen transform")
    ...
    if not is_child:
        ...
        print("Scientific change: persistent transformer hotset H8 -> H24 ONLY")
        print("M=5 oracle geometry / runtime / model / KV / parity / I-O / safety: UNCHANGED")

    stretch017 = load_module(source017, "loom_stretch017_transform_h24")
    ...
    if missing:
        raise RuntimeError(f"H24 transformed-wrapper invariant failed; missing {missing}")
