#!/usr/bin/env python3
"""LOOM Stretch 021 helper — M5 with thirty-two-layer persistent hotset.

Reuses the frozen Stretch 017 M=5 exact oracle path and changes one runtime
factor only: persistent transformer hotset layers 0..7 -> 0..31. All model,
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"H32 transform failed for {label}: expected 1 occurrence, found {count}")
...
def transformed_013_wrapper(source013: str, stretch017) -> str:
    ...
    # checked by the balanced Stretch 021 parent using hotset provenance.
    transformed = transformed.replace(
        "LOOM Stretch 017 — Five-Token Oracle Block Confirmation",
        "LOOM Stretch 021 — Five-Token H32 Hotset Variant",
    )
    transformed = transformed.replace(
        "Stretch 017 — Five-Token Oracle Block Confirmation",
        "Stretch 021 — Five-Token H32 Hotset Variant",
    )
    transformed = transformed.replace(
        "five-token-oracle-block-confirmation-017",
        "five-token-h32-hotset-021-h32",
    )
    ...
    transformed = replace_once(
        ...
        '        "HOTSET_LAYER_IDS = tuple(range(32))",\n',
        "required H32 fragment",
    )
    ...
    # only because it is the derived residency/provenance gate for H32.
    ...
    replacement = (
        ...
        "        \"HOTSET_LAYER_IDS = tuple(range(32))\\nEXPECTED_HOTSET_BYTES = 32 * EXPECTED_LAYER_BYTES\\n\",\n"
        "        1,\n"
        "        \"thirty-two-layer persistent hotset\",\n"
        "    )\n"
        "    source = add_oracle_block_transform(source)\n"
    )
    transformed = replace_once(transformed, anchor, replacement, "H32 injection")

    transformed = transformed.replace(
        'print("Eight-layer persistent hotset: PRESERVED")',
        'print("Persistent hotset: H32 layers 0..31 (only changed runtime factor)")',
    )
    return transformed
...
def main() -> int:
    ...
    if not is_child:
        print("LOOM Stretch 021 — Five-Token H32 Hotset Variant frozen transform")
    ...
    if not is_child:
        ...
        print("Scientific change: persistent transformer hotset H8 -> H32 ONLY")
        print("M=5 oracle geometry / runtime / model / KV / parity / I-O / safety: UNCHANGED")

    stretch017 = load_module(source017, "loom_stretch017_transform_h32")
    ...
    if missing:
        raise RuntimeError(f"H32 transformed-wrapper invariant failed; missing {missing}")
