#!/usr/bin/env python3
"""Shared Stretch 031 Fix2 renderer and preflight.
...
LAUNCHER_PATHS = {
    "M5": Path("scripts/stretch_single_pass_m5_ten_token_control_031_fix2.py"),
    "M2": Path("scripts/stretch_single_pass_m2_ten_token_variant_031_fix2.py"),
}
...
VARIANTS = {
    "M5": {
        ...
        "run_directory": "m5-ten-token-single-pass-control-031-fix2",
    },
    "M2": {
        "block_size": 2,
        "block_count": 5,
        ...
        "run_directory": "m2-ten-token-single-pass-treatment-031-fix2",
    },
}
...
def replace_once(source: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"Stretch 031 Fix2 transform failed for {label}: expected 1 occurrence, found {count}")
    return source.replace(old, new, 1)


def replace_all(source: str, old: str, new: str, label: str) -> str:
    if old not in source:
        raise RuntimeError(f"Stretch 031 Fix2 transform failed for {label}: source text missing")
...
def add_geometry(source: str, variant: str) -> str:
    ...
    # summary identity, which is the exact failing literal observed by Fix2.
    source = replace_all(
        source,
...
def inherited_modules(repo: Path) -> tuple[Any, dict[str, Any], dict[str, Path]]:
    ...
    if observed027 != SOURCE_027_BLOB:
        raise RuntimeError(f"Stretch 031 Fix2 provenance failure: Stretch027={observed027}")
    s027 = load_module(s027_path, "loom_stretch027_for_031_fix2")
    paths = {
    ...
    if observed != expected:
        raise RuntimeError(f"Stretch 031 Fix2 inherited provenance failure: observed={observed} expected={expected}")
    modules = {
        "s026": load_module(paths["s026"], "loom_stretch026_for_031_fix2"),
        "s025": load_module(paths["s025"], "loom_stretch025_for_031_fix2"),
        "fix1": load_module(paths["fix1"], "loom_stretch023_fix1_for_031_fix2"),
        "broken": load_module(paths["broken"], "loom_stretch023_broken_for_031_fix2"),
        "h36": load_module(paths["h36"], "loom_stretch022_h36_for_031_fix2"),
        "s017": load_module(paths["s017"], "loom_stretch017_for_031_fix2"),
    }
...
def inject_geometry_callback(wrapper: str, variant: str) -> tuple[str, dict[str, int]]:
    ...
    if wrapper.count(anchor) != 1:
        raise RuntimeError("Stretch 031 Fix2 wrapper invariant failed: inherited post-preflight anchor missing")
    ...
    if wrapper.count(final_exec) != 1:
        raise RuntimeError("Stretch 031 Fix2 wrapper invariant failed: final runtime execution anchor missing")
    final_exec_position = wrapper.index(final_exec)
    if not inherited_position < callback_position < final_exec_position:
        raise RuntimeError("Stretch 031 Fix2 wrapper callback phase invariant failed")
    return wrapper, {
...
def build_wrapper(repo: Path, variant: str) -> tuple[str, dict[str, Any], dict[str, Path], Any, dict[str, Any]]:
    ...
    if missing:
        raise RuntimeError(f"Stretch 031 Fix2 wrapper invariant failed: missing {missing}")
...
def render_final_source(repo: Path, variant: str) -> tuple[str, dict[str, Any]]:
    ...
    if wrapper.count(trailer) != 1:
        raise RuntimeError("Stretch 031 Fix2 render invariant failed: wrapper main trailer missing")
    ...
    namespace["main"]()
    if len(captured) != 1:
        raise RuntimeError(f"Stretch 031 Fix2 render failed: captured {len(captured)} final sources")
    source = captured[0]
...
def source_facts(source: str, variant: str) -> dict[str, Any]:
    ...
    if missing or forbidden:
        raise RuntimeError(f"Stretch 031 Fix2 final-source invariant {variant} failed: missing={missing} forbidden={forbidden}")
    return {
        "variant": variant,
...
def dispatch_preflight_child(variant: str, argv: list[str]) -> int:
    """Child-side no-model sentinel, reached through the real Fix2 shim."""
    if len(argv) != 2:
    ...
    versions = {name: metadata.version(name) for name in EXPECTED_RUNTIME}
    if versions != EXPECTED_RUNTIME:
        raise RuntimeError(f"dispatch child runtime mismatch: {versions} != {EXPECTED_RUNTIME}")
    marker = {
        "kind": "STRETCH_031_FIX2_DISPATCH_CHILD_MARKER",
        "no_model_load": True,
...
def dispatch_preflight(repo: Path, variant: str, output_dir: Path) -> dict[str, Any]:
    """Run the explicit Fix2 parent -> canonical-child no-model transport."""
    cfg = VARIANTS[variant]
    launcher = (repo / LAUNCHER_PATHS[variant]).resolve()
    if not launcher.is_file():
        raise RuntimeError(f"Fix2 launcher missing: {launcher}")
    ...
    if missing:
        raise RuntimeError(f"Fix2 launcher dispatch invariant failed {variant}: missing {missing}")
    child_python = (repo / CANONICAL_CHILD_PYTHON).resolve()
    ...
    parent = {
        "kind": "STRETCH_031_FIX2_DISPATCH_PARENT",
        "no_model_load": True,
        "outer_python_executable": sys.executable,
        "outer_script_path": str(launcher),
        "child_python_executable": str(child_python),
...
def preflight(repo: Path, output_dir: Path | None = None) -> dict[str, Any]:
    if output_dir is None:
        raise RuntimeError("Fix2 preflight requires an output directory for durable dispatch markers")
    output_dir.mkdir(parents=True, exist_ok=False)
    sources: dict[str, str] = {}
    result: dict[str, Any] = {
        "experiment": "Stretch 031 Fix2 local generated-source and dispatch preflight",
    ...
    if normalized_m5 != sources["M2"]:
        raise RuntimeError("Stretch 031 Fix2 single-factor invariant failed: normalized M5 source differs from M2")
    ...
    result["classification"] = "STRETCH_031_FIX2_PREFLIGHT_PASS"
...
def default_preflight_dir(repo: Path) -> Path:
    ...
    return repo / "results-local/stretch/single-pass-m2-m5-geometry-comparison-031-fix2/preflight" / stamp


def run_variant(variant: str, launcher_path: Path, argv: list[str] | None = None) -> int:
    """Run a Fix2 variant with its explicit executable shim identity."""
    ...
    if launcher_path != expected_launcher:
        raise RuntimeError(f"Fix2 explicit launcher mismatch: {launcher_path} != {expected_launcher}")
    if not launcher_path.is_file():
        raise RuntimeError(f"Fix2 launcher missing: {launcher_path}")
