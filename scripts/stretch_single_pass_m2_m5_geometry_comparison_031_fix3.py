#!/usr/bin/env python3
"""LOOM Stretch 031 — balanced M2 vs M5 geometry comparison, harness Fix3.

Fix3 never reuses failed constituents. It validates rendered sources plus the
explicit parent -> child no-model dispatch before any scientific MLX launch.
"""
...
from stretch_single_pass_geometry_harness_031_fix3 import default_preflight_dir, preflight

SOURCE_M5_PATH = Path("scripts/stretch_single_pass_m5_ten_token_control_031_fix3.py")
SOURCE_M5_BLOB = "c0803e729fdfe33020809068b087230693f4eebe"
SOURCE_M2_PATH = Path("scripts/stretch_single_pass_m2_ten_token_variant_031_fix3.py")
SOURCE_M2_BLOB = "78ba417bf9efc0f827adc429b9b9adf26744488a"
...
def main() -> int:
    ...
    args = sys.argv[1:]
    if args:
        if args not in (["--preflight"], ["--render-only"]):
            print(f"usage: {Path(sys.argv[0]).name} [--preflight|--render-only]", file=sys.stderr)
            return 2
        preflight_dir = default_preflight_dir(repo)
        try:
            result = preflight(repo, preflight_dir)
        except Exception as exc:
            print(f"Classification: STRETCH_031_FIX3_PREFLIGHT_FAIL", file=sys.stderr)
            print(f"Failure reason: {type(exc).__name__}: {exc}", file=sys.stderr)
            return 2
        print(f"Classification: {result['classification']}")
        print(f"Scientific run: {result['scientific_run']}")
        print(f"Preflight summary: {result['summary']}")
        return 0
    ...
    preflight_dir = default_preflight_dir(repo)
    try:
        preflight_result = preflight(repo, preflight_dir)
    except Exception as exc:
        print("Classification: STRETCH_031_FIX3_PREFLIGHT_FAIL", file=sys.stderr)
        print(f"Failure reason: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    print(f"Preflight: {preflight_result['classification']} ({preflight_result['summary']})")
    ...
    run_dir = repo / "results-local" / "stretch" / "single-pass-m2-m5-geometry-comparison-031-fix3" / run_id
    run_dir.mkdir(parents=True, exist_ok=False)
    summary_path = run_dir / "summary.json"
    summary: dict = {
        "experiment": "Stretch 031 — Balanced M2 vs M5 SINGLE_PASS Geometry Comparison Fix3", 
        "harness_revision": "Fix3: literal canonical venv launcher + guarded no-model parent/child transport", 
