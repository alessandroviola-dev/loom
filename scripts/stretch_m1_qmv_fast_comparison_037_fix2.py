...
def fix1_normalized_harness_diff(repo: Path) -> dict:
    """Prove Fix2 differs from Fix1 only in revision identity and output paths."""
    fix1 = (repo / "scripts/stretch_m1_qmv_fast_comparison_037_fix1.py").read_text()
    fix2 = Path(__file__).read_text()
    normalized = re.sub(
        r'\ndef fix1_normalized_harness_diff\(repo: Path\) -> dict:.*?\n\ndef parse_summary',
        '\n\ndef parse_summary', fix2, flags=re.S,
    ).replace(f'HARNESS_REVISION = "{HARNESS_REVISION}"\n', '').replace(
        '    normalized_harness_diff = fix1_normalized_harness_diff(repo)\n', ''
    )
    normalized = re.sub(r'\n{3,}def parse_summary', '\n\n\ndef parse_summary', normalized)
    normalized = normalized.replace("m1-qmv-fast-tuning-037-fix2", "m1-qmv-fast-tuning-037")
    normalized = normalized.replace(
        '"classification":"STRETCH_037_PREFLIGHT_PASS","harness_revision":HARNESS_REVISION,"fix1_to_fix2_normalized_harness_diff":normalized_harness_diff,',
        '"classification":"STRETCH_037_PREFLIGHT_PASS",'
    ).replace(
        '"experiment":"Stretch 037 built-in MLX qmv_fast vs M1-specific custom S1_R8","harness_revision":HARNESS_REVISION,"classification":"RUNNING",',
        '"experiment":"Stretch 037 built-in MLX qmv_fast vs M1-specific custom S1_R8","classification":"RUNNING",'
    )
    if normalized != fix1:
        raise RuntimeError("Fix1 -> Fix2 normalized harness diff failed")
    return {"status": "PASS", "allowed_differences": ["HARNESS_REVISION", "Fix2 output paths"]}
...
def render_preflight(repo: Path, root: Path) -> dict:
    ...
    return {"classification":"STRETCH_037_PREFLIGHT_PASS","harness_revision":HARNESS_REVISION,"fix1_to_fix2_normalized_harness_diff":normalized_harness_diff,"scientific_run":False,"py_compile":"PASS","control_source_sha256":cf["control_sha256"],"treatment_source_sha256":tf["treatment_sha256"],"normalized_scientific_diff":"PASS: only built-in MLX qmv implementation vs custom S1_R8 injection","static_kernel_invariants":"PASS","literal_launcher":str(launcher),"no_model_dispatch":marker,"runtime_expected":EXPECTED_VERSIONS,"startup":startup,"directory":str(root)}

def main() -> int:
    repo=Path(__file__).resolve().parents[1]
    if sys.argv[1:] == ["--preflight"]:
        root=repo/"results-local/stretch/m1-qmv-fast-tuning-037-fix2/preflight"/datetime.now().strftime("%Y%m%d-%H%M%S"); root.mkdir(parents=True)
    ...
    root=repo/"results-local/stretch/m1-qmv-fast-tuning-037-fix2"/datetime.now().strftime("%Y%m%d-%H%M%S"); root.mkdir(parents=True)
    preflight_dir=root/"preflight"; preflight_dir.mkdir(parents=True,exist_ok=False)
    try: preflight=render_preflight(repo,preflight_dir); (preflight_dir/"preflight-summary.json").write_text(json.dumps(preflight,indent=2)+"\n")
    except Exception as exc:
