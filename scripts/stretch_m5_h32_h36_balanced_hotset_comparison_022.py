#!/usr/bin/env python3
"""LOOM Stretch 022 — balanced M5 H32 vs H36 hotset comparison.

Runs the frozen exact M=5 target path with either 32 or 36 persistent
...
SOURCE_H32_PATH = Path("scripts/stretch_five_token_h32_hotset_variant_021.py")
SOURCE_H32_BLOB = "b6b39dfb095b905ed659d52903309783efc02be7"
...
def main() -> int:
    ...
    h32_path = repo / SOURCE_H32_PATH
    h36_path = repo / SOURCE_H36_PATH
    observed_h32 = git_blob(h32_path, repo)
    observed_h36 = git_blob(h36_path, repo)

    print("LOOM Stretch 022 — Balanced M5 H32 vs H36 Hotset Comparison")
    print(f"Stretch 021 H32 helper blob: {observed_h32}")
    print(f"Stretch 022 H36 helper blob: {observed_h36}")
    if observed_h32 != SOURCE_H32_BLOB or observed_h36 != SOURCE_H36_BLOB:
        print("Source provenance: FAIL", file=sys.stderr)
        return 2
    print("Source provenance: PASS")
    print("Balanced run order: H32 -> H36 -> H36 -> H32")
    print("Frozen exact oracle block size: M=5")
    print("Scientific factor: persistent transformer layers 32 -> 36 ONLY")
    ...
    root = repo / "results-local" / "stretch" / "m5-h32-h36-balanced-hotset-comparison-022"
    ...
    summary: dict = {
        "experiment": "Stretch 022 — Balanced M5 H32 vs H36 Hotset Comparison",
        "classification": "RUNNING",
        "run_id": run_id,
        "source_provenance": {
            "h32_path": str(SOURCE_H32_PATH),
            "h32_expected_blob": SOURCE_H32_BLOB,
            "h32_observed_blob": observed_h32,
            "h36_path": str(SOURCE_H36_PATH),
            "h36_expected_blob": SOURCE_H36_BLOB,
            "h36_observed_blob": observed_h36,
        },
        "run_order": RUN_ORDER,
        "oracle_block_size": 5,
        "scientific_factor": "persistent transformer hotset H32 -> H36 only",
    ...
    save()
    scripts = {"H32": h32_path, "H36": h36_path}
    ...
    h32_records = [r for r in summary["attempts"] if r["variant"] == "H32"]
    h36_records = [r for r in summary["attempts"] if r["variant"] == "H36"]
    h32 = aggregate_variant(h32_records)
    h36 = aggregate_variant(h36_records)

    comparison = {
        "h32": h32,
        "h36": h36,
        "h36_over_h32_pooled_target_rate_ratio": safe_ratio(
            h36["pooled_target_verification_tokens_per_second"],
            h32["pooled_target_verification_tokens_per_second"],
        ),
        "h36_over_h32_median_block_wall_ratio": safe_ratio(
            h36["median_block_full_pass_seconds"], h32["median_block_full_pass_seconds"]
        ),
        "h36_over_h32_median_materialize_ratio": safe_ratio(
            h36["median_block_materialize_seconds"], h32["median_block_materialize_seconds"]
        ),
        "h36_over_h32_median_forward_ratio": safe_ratio(
            h36["median_block_forward_seconds"], h32["median_block_forward_seconds"]
        ),
        "h36_over_h32_mean_full_pass_read_bytes_per_block_ratio": safe_ratio(
            h36["mean_full_pass_disk_read_bytes_per_block"], h32["mean_full_pass_disk_read_bytes_per_block"]
        ),
        "h36_over_h32_mean_materialize_read_bytes_per_block_ratio": safe_ratio(
            h36["mean_materialize_disk_read_bytes_per_block"], h32["mean_materialize_disk_read_bytes_per_block"]
        ),
        "h36_over_h32_hotset_bytes_ratio": safe_ratio(
            h36["persistent_hotset_bytes"], h32["persistent_hotset_bytes"]
        ),
    }
    comparison["observed_higher_pooled_target_rate"] = (
        "H36"
        if h36["pooled_target_verification_tokens_per_second"] > h32["pooled_target_verification_tokens_per_second"]
        else "H32"
    )

    summary["comparison"] = comparison
    summary["classification"] = "M5_H32_H36_BALANCED_HOTSET_COMPARISON_PASS"
    summary["disk_free_after_gib"] = gib_free(repo)
    save()

    print("Classification: M5_H32_H36_BALANCED_HOTSET_COMPARISON_PASS")
    print(f"H32 pooled target-verification tok/s: {h32['pooled_target_verification_tokens_per_second']}")
    print(f"H36 pooled target-verification tok/s: {h36['pooled_target_verification_tokens_per_second']}")
    print(f"H36/H32 target-rate ratio: {comparison['h36_over_h32_pooled_target_rate_ratio']}")
    print(f"H32 median block wall: {h32['median_block_full_pass_seconds']} s")
    print(f"H36 median block wall: {h36['median_block_full_pass_seconds']} s")
    print(f"H36/H32 median materialize ratio: {comparison['h36_over_h32_median_materialize_ratio']}")
    print(f"H36/H32 median forward ratio: {comparison['h36_over_h32_median_forward_ratio']}")
    print(f"H36/H32 mean full-pass process-read bytes/block ratio: {comparison['h36_over_h32_mean_full_pass_read_bytes_per_block_ratio']}")
    print(f"H32 persistent hotset bytes: {h32['persistent_hotset_bytes']}")
    print(f"H36 persistent hotset bytes: {h36['persistent_hotset_bytes']}")
    print(f"H32 hybrid raw-weight budget bytes: {h32['hybrid_raw_weight_budget_bytes']}")
    print(f"H36 hybrid raw-weight budget bytes: {h36['hybrid_raw_weight_budget_bytes']}")
    print(f"H32 minimum observed free memory: {h32['minimum_observed_free_memory_percent']}%")
    print(f"H36 minimum observed free memory: {h36['minimum_observed_free_memory_percent']}%")
    print(f"H32 peak observed swap: {h32['peak_observed_swap_mb']} MB")
    print(f"H36 peak observed swap: {h36['peak_observed_swap_mb']} MB")
