...
def inject_geometry_callback(wrapper_source: str) -> str:
    # Geometry is applied after the inherited Stretch 017/H36 wrapper has
    # validated its frozen M5 reconstruction. This preserves provenance while
    # allowing the final runtime source to exercise the preregistered M2 factor.
    anchor = (
    ...
    if wrapper_source.count(anchor) != 1:
        raise RuntimeError("Stretch 031 M2 wrapper invariant failed: post-preflight injection anchor missing")
