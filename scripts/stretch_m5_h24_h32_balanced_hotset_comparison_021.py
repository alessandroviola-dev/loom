#!/usr/bin/env python3
"""LOOM Stretch 021 — balanced M5 H24 vs H32 hotset comparison.

Runs the frozen exact M=5 target path with either 24 or 32 persistent
...
def main() -> int:
    ...
    print("LOOM Stretch 021 — Balanced M5 H24 vs H32 Hotset Comparison")
    print(f"Stretch 020 H24 helper blob: {observed_h24}")
    print(f"Stretch 021 H32 helper blob: {observed_h32}")
    if observed_h24 != SOURCE_H24_BLOB or observed_h32 != SOURCE_H32_BLOB:
        print("Source provenance: FAIL", file=sys.stderr)
        return 2
    print("Source provenance: PASS")
    print("Balanced run order: H24 -> H32 -> H32 -> H24")
    print("Frozen exact oracle block size: M=5")
    print("Scientific factor: persistent transformer layers 24 -> 32 ONLY")
    ...
    root = repo / "results-local" / "stretch" / "m5-h24-h32-balanced-hotset-comparison-021"
    ...
    summary: dict = {
        "experiment": "Stretch 021 — Balanced M5 H24 vs H32 Hotset Comparison",
        ...
        "run_order": RUN_ORDER,
        "oracle_block_size": 5,
        "scientific_factor": "persistent transformer hotset H24 -> H32 only",
    ...
    save()
    scripts = {"H24": h24_path, "H32": h32_path}
    ...
    comparison = {
        "h24": h24,
        "h32": h32,
        "h32_over_h24_pooled_target_rate_ratio": safe_ratio(
            h32["pooled_target_verification_tokens_per_second"],
            h24["pooled_target_verification_tokens_per_second"],
        ),
        "h32_over_h24_median_block_wall_ratio": safe_ratio(
            h32["median_block_full_pass_seconds"], h24["median_block_full_pass_seconds"]
        ),
        "h32_over_h24_median_materialize_ratio": safe_ratio(
            h32["median_block_materialize_seconds"], h24["median_block_materialize_seconds"]
        ),
        "h32_over_h24_median_forward_ratio": safe_ratio(
            h32["median_block_forward_seconds"], h24["median_block_forward_seconds"]
        ),
        "h32_over_h24_mean_full_pass_read_bytes_per_block_ratio": safe_ratio(
            h32["mean_full_pass_disk_read_bytes_per_block"], h24["mean_full_pass_disk_read_bytes_per_block"]
        ),
        "h32_over_h24_mean_materialize_read_bytes_per_block_ratio": safe_ratio(
            h32["mean_materialize_disk_read_bytes_per_block"], h24["mean_materialize_disk_read_bytes_per_block"]
        ),
        "h32_over_h24_hotset_bytes_ratio": safe_ratio(
            h32["persistent_hotset_bytes"], h24["persistent_hotset_bytes"]
        ),
    }
    comparison["observed_higher_pooled_target_rate"] = (
        "H32"
        if h32["pooled_target_verification_tokens_per_second"] > h24["pooled_target_verification_tokens_per_second"]
        else "H24"
    )

    summary["comparison"] = comparison
    summary["classification"] = "M5_H24_H32_BALANCED_HOTSET_COMPARISON_PASS"
    summary["disk_free_after_gib"] = gib_free(repo)
    save()

    print("Classification: M5_H24_H32_BALANCED_HOTSET_COMPARISON_PASS")
    ...
    print(f"H32/H24 target-rate ratio: {comparison['h32_over_h24_pooled_target_rate_ratio']}")
    ...
    print(f"H32/H24 median materialize ratio: {comparison['h32_over_h24_median_materialize_ratio']}")
    print(f"H32/H24 median forward ratio: {comparison['h32_over_h24_median_forward_ratio']}")
    print(f"H32/H24 mean full-pass process-read bytes/block ratio: {comparison['h32_over_h24_mean_full_pass_read_bytes_per_block_ratio']}")
