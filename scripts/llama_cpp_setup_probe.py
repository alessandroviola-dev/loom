...
def run(args: list[str], cwd: Path, timeout: int = 60, env: dict | None = None) -> dict:
    try:
        proc = subprocess.run(
            args,
            cwd=cwd,
...
def cmd_version(cmd: list[str], cwd: Path) -> str:
    result = run(cmd, cwd)
    text = (result.get("stdout", "") + result.get("stderr", "")).strip()
    return text
...
def main() -> int:
    ...
    summary["toolchain"] = {
        ...
        "cmake": cmd_version(["cmake", "--version"], repo_root) if not missing else "",
    ...
    if missing:
        ...
        summary["failure_reason"] = f"missing prerequisites: {', '.join(missing)}"
        summary["finished_at_utc"] = utc_now()
        path = out_dir / "setup-summary.json"
        path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print("LOOM llama.cpp Phase 4 setup probe")
        print(f"Prerequisites: FAIL — missing {', '.join(missing)}")
    ...
    if not source.exists():
        ...
        if clone.get("exit_code") != 0:
            summary["success"] = False
            summary["failure_reason"] = "git clone failed"
            summary["finished_at_utc"] = utc_now()
            path = out_dir / "setup-summary.json"
            path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
            print("LOOM llama.cpp Phase 4 setup probe")
            print("Clone: FAIL")
            print(clone.get("stderr", "").strip())
            print(f"Summary: {path}")
            return 1
    ...
    if fetch.get("exit_code") != 0:
        summary["success"] = False
        summary["failure_reason"] = "git fetch pinned commit failed"
        summary["finished_at_utc"] = utc_now()
        path = out_dir / "setup-summary.json"
        path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
        print("LOOM llama.cpp Phase 4 setup probe")
        print("Fetch pinned commit: FAIL")
        print(fetch.get("stderr", "").strip())
        print(f"Summary: {path}")
        return 1
    ...
    metal = find_cache_value(cache, "GGML_METAL")
    embed = find_cache_value(cache, "GGML_METAL_EMBED_LIBRARY")
    ...
    summary["build_verification"] = {
        ...
        "GGML_METAL": metal,
        "GGML_METAL_EMBED_LIBRARY": embed,
    ...
    print(f"Prerequisites: {'PASS' if not missing else 'FAIL'}")
    ...
    print(f"GGML_METAL: {metal}")
    print(f"GGML_METAL_EMBED_LIBRARY: {embed}")
