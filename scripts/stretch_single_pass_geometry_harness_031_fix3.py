#!/usr/bin/env python3
"""Shared Stretch 031 Fix3 renderer and preflight.
...
CANONICAL_VENV_PREFIX = Path("results-local/mlx/venv-mlx-lm-0.31.3")
LAUNCHER_PATHS = {
    "M5": Path("scripts/stretch_single_pass_m5_ten_token_control_031_fix3.py"),
    "M2": Path("scripts/stretch_single_pass_m2_ten_token_variant_031_fix3.py"),
}
...
VARIANTS = {
    "M5": {
        ...
        "run_directory": "m5-ten-token-single-pass-control-031-fix3",
    },
    "M2": {
        "block_size": 2,
        "block_count": 5,
        ...
        "run_directory": "m2-ten-token-single-pass-treatment-031-fix3",
    },
}
...
def replace_once(source: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"Stretch 031 Fix3 transform failed for {label}: expected 1 occurrence, found {count}")
    return source.replace(old, new, 1)


def replace_all(source: str, old: str, new: str, label: str) -> str:
    if old not in source:
        raise RuntimeError(f"Stretch 031 Fix3 transform failed for {label}: source text missing")
...
def add_geometry(source: str, variant: str) -> str:
    ...
    # summary identity, which is the exact failing literal observed by Fix3.
    source = replace_all(
        source,
...
def inherited_modules(repo: Path) -> tuple[Any, dict[str, Any], dict[str, Path]]:
    ...
    if observed027 != SOURCE_027_BLOB:
        raise RuntimeError(f"Stretch 031 Fix3 provenance failure: Stretch027={observed027}")
    s027 = load_module(s027_path, "loom_stretch027_for_031_fix3")
    paths = {
    ...
    if observed != expected:
        raise RuntimeError(f"Stretch 031 Fix3 inherited provenance failure: observed={observed} expected={expected}")
    modules = {
        "s026": load_module(paths["s026"], "loom_stretch026_for_031_fix3"),
        "s025": load_module(paths["s025"], "loom_stretch025_for_031_fix3"),
        "fix1": load_module(paths["fix1"], "loom_stretch023_fix1_for_031_fix3"),
        "broken": load_module(paths["broken"], "loom_stretch023_broken_for_031_fix3"),
        "h36": load_module(paths["h36"], "loom_stretch022_h36_for_031_fix3"),
        "s017": load_module(paths["s017"], "loom_stretch017_for_031_fix3"),
    }
...
def inject_geometry_callback(wrapper: str, variant: str) -> tuple[str, dict[str, int]]:
    ...
    if wrapper.count(anchor) != 1:
        raise RuntimeError("Stretch 031 Fix3 wrapper invariant failed: inherited post-preflight anchor missing")
    ...
    if wrapper.count(final_exec) != 1:
        raise RuntimeError("Stretch 031 Fix3 wrapper invariant failed: final runtime execution anchor missing")
    final_exec_position = wrapper.index(final_exec)
    if not inherited_position < callback_position < final_exec_position:
        raise RuntimeError("Stretch 031 Fix3 wrapper callback phase invariant failed")
    return wrapper, {
...
def build_wrapper(repo: Path, variant: str) -> tuple[str, dict[str, Any], dict[str, Path], Any, dict[str, Any]]:
    ...
    if missing:
        raise RuntimeError(f"Stretch 031 Fix3 wrapper invariant failed: missing {missing}")
...
def render_final_source(repo: Path, variant: str) -> tuple[str, dict[str, Any]]:
    ...
    if wrapper.count(trailer) != 1:
        raise RuntimeError("Stretch 031 Fix3 render invariant failed: wrapper main trailer missing")
    ...
    namespace["main"]()
    if len(captured) != 1:
        raise RuntimeError(f"Stretch 031 Fix3 render failed: captured {len(captured)} final sources")
    source = captured[0]
...
def source_facts(source: str, variant: str) -> dict[str, Any]:
    ...
    required = {
        ...
        "parent_child_dispatch": "script_path = Path(__file__).resolve()",
    }
    ...
    if missing or forbidden:
        raise RuntimeError(f"Stretch 031 Fix3 final-source invariant {variant} failed: missing={missing} forbidden={forbidden}")
    return {
        "variant": variant,
...
def selected_control_python(repo: Path) -> Path:
    """Return the literal venv launcher; never resolve this executable path."""
    path = repo / CANONICAL_CHILD_PYTHON
    if not path.is_file():
        raise RuntimeError(f"canonical MLX child interpreter missing: {path}")
    expected = repo / "results-local/mlx/venv-mlx-lm-0.31.3/bin/python"
    if path != expected:
        raise RuntimeError(f"canonical launcher path invariant failed: {path} != {expected}")
    return path


def child_interpreter_info(repo: Path) -> dict[str, Any]:
    path = selected_control_python(repo)
    query = (
        "import importlib.metadata as m,json,sys; "
        "print(json.dumps({'executable':sys.executable,'prefix':sys.prefix,"
        "'versions':{k:m.version(k) for k in ('mlx','mlx-metal','mlx-lm','transformers')}}))"
    )
    # `path` intentionally remains the venv/bin/python symlink string.
    ...
    expected_prefix = str(repo / CANONICAL_VENV_PREFIX)
    if info.get("prefix") != expected_prefix:
        raise RuntimeError(f"canonical child prefix mismatch: {info.get('prefix')} != {expected_prefix}")
    if info.get("versions") != EXPECTED_CONTROL_RUNTIME:
        raise RuntimeError(f"canonical child runtime mismatch: {info.get('versions')} != {EXPECTED_CONTROL_RUNTIME}")
    return {"configured_launcher_path": str(path), "expected_prefix": expected_prefix, "query": info}


def assert_no_venv_python_dereference(repo: Path) -> dict[str, Any]:
    """Static regression guard: venv Python launchers must never be resolved."""
    paths = [
        repo / "scripts/stretch_single_pass_geometry_harness_031_fix3.py",
        repo / "scripts/stretch_single_pass_m5_ten_token_control_031_fix3.py",
        repo / "scripts/stretch_single_pass_m2_ten_token_variant_031_fix3.py",
        repo / "scripts/stretch_single_pass_m2_m5_geometry_comparison_031_fix3.py",
        repo / "scripts/test_stretch_single_pass_venv_dispatch_031_fix3.py",
    ]
    forbidden = [
        r"bin/python[\"']?\)\.resolve\(",
        r"CANONICAL_CHILD_PYTHON\)\.resolve\(",
    ...
    if hits:
        raise RuntimeError(f"venv launcher dereference regression guard failed: {hits}")
    return {"pass": True, "scanned_files": [str(path) for path in paths], "forbidden_patterns": forbidden}


def dispatch_preflight_child(variant: str, argv: list[str]) -> int:
    """Child-side no-model sentinel, reached through the real Fix3 shim."""
    if len(argv) != 2:
        raise RuntimeError("dispatch-preflight child expected marker path and metadata JSON")
    marker_path = Path(argv[0])
    parent = json.loads(argv[1])
    cfg = VARIANTS[variant]
    actual_script = Path(sys.argv[0]).resolve()
    expected_script = Path(parent["expected_child_script"]).resolve()
    if actual_script != expected_script:
        raise RuntimeError(f"dispatch child script mismatch: {actual_script} != {expected_script}")
    if parent.get("expected_variant") != variant:
        raise RuntimeError(f"dispatch child variant mismatch: {parent.get('expected_variant')} != {variant}")
    loaded_mlx = sorted(name for name in sys.modules if name == "mlx" or name.startswith("mlx."))
    if loaded_mlx:
        raise RuntimeError(f"dispatch preflight must not import MLX: {loaded_mlx}")
    versions = {name: metadata.version(name) for name in EXPECTED_CONTROL_RUNTIME}
    expected_prefix = parent.get("expected_child_prefix")
    if sys.prefix != expected_prefix:
        raise RuntimeError(f"dispatch child prefix mismatch: {sys.prefix} != {expected_prefix}")
    if versions != EXPECTED_CONTROL_RUNTIME:
        raise RuntimeError(f"dispatch child runtime mismatch: {versions} != {EXPECTED_CONTROL_RUNTIME}")
    marker = {
        "kind": "STRETCH_031_FIX3_DISPATCH_CHILD_MARKER",
        "model_loaded": False,
        "target_compute_executed": False,
        "mlx_modules_loaded": loaded_mlx,
        "expected_variant": variant,
        "observed_oracle_block_size": cfg["block_size"],
        "observed_oracle_block_count": cfg["block_count"],
        "generated_tokens": 10,
        "oracle_prefix": TEN_TOKEN_ORACLE,
        "oracle_prefix_sha256": hashlib.sha256(TEN_TOKEN_ORACLE.encode("utf-8")).hexdigest(),
        "launcher_path_received": parent["child_launcher_path"],
        "invoking_shim_path": str(actual_script),
        "child_python_executable": sys.executable,
        "child_script_path": str(actual_script),
...
def dispatch_preflight(repo: Path, variant: str, output_dir: Path) -> dict[str, Any]:
    """Run the explicit Fix3 parent -> canonical-child no-model transport."""
    cfg = VARIANTS[variant]
    launcher = (repo / LAUNCHER_PATHS[variant]).resolve()
    if not launcher.is_file():
        raise RuntimeError(f"Fix3 launcher missing: {launcher}")
    launcher_text = launcher.read_text(encoding="utf-8")
    required_launcher = [
        "dispatch_preflight_child",
        "launcher_path=Path(__file__).resolve()",
        f'run_variant("{variant}"',
    ]
    missing = [fragment for fragment in required_launcher if fragment not in launcher_text]
    if missing:
        raise RuntimeError(f"Fix3 launcher dispatch invariant failed {variant}: missing {missing}")
    # Do not resolve this venv executable: its symlink identity is required.
    child_python = selected_control_python(repo)
    marker_path = output_dir / f"dispatch-{variant.lower()}-child-marker.json"
    parent_path = output_dir / f"dispatch-{variant.lower()}-parent.json"
    if marker_path.exists() or parent_path.exists():
        raise RuntimeError(f"dispatch preflight paths already exist for {variant}")
    parent = {
        "kind": "STRETCH_031_FIX3_DISPATCH_PARENT",
        "model_loaded": False,
        "target_compute_executed": False,
        "outer_python_executable": sys.executable,
        "outer_script_path": str(launcher),
        "child_launcher_path": str(child_python),
        "expected_child_script": str(launcher),
        "expected_child_prefix": str(repo / CANONICAL_VENV_PREFIX),
        "expected_variant": variant,
        "observed_oracle_block_size": cfg["block_size"],
        "observed_oracle_block_count": cfg["block_count"],
        "generated_tokens": 10,
        "oracle_prefix": TEN_TOKEN_ORACLE,
        "oracle_prefix_sha256": hashlib.sha256(TEN_TOKEN_ORACLE.encode("utf-8")).hexdigest(),
        "runtime_versions": EXPECTED_CONTROL_RUNTIME,
    }
    parent_path.write_text(json.dumps(parent, indent=2, sort_keys=True), encoding="utf-8")
    command = [str(child_python), str(launcher), "--dispatch-preflight-child", str(marker_path), json.dumps(parent, sort_keys=True)]
    proc = subprocess.run(command, cwd=repo, text=True, capture_output=True, timeout=60, check=False)
    if proc.returncode != 0:
        raise RuntimeError(f"dispatch preflight child failed {variant}: rc={proc.returncode} stderr={proc.stderr.strip()}")
    if not marker_path.is_file():
        raise RuntimeError(f"dispatch preflight child marker missing {variant}")
    ...
    expected_marker = {
        ...
        "oracle_prefix": TEN_TOKEN_ORACLE,
        "oracle_prefix_sha256": hashlib.sha256(TEN_TOKEN_ORACLE.encode("utf-8")).hexdigest(),
        "launcher_path_received": str(child_python),
        "invoking_shim_path": str(launcher),
        "child_script_path": str(launcher),
        "sys_prefix": str(repo / CANONICAL_VENV_PREFIX),
        "runtime_versions": EXPECTED_CONTROL_RUNTIME,
    }
    mismatches = {key: (marker.get(key), expected) for key, expected in expected_marker.items() if marker.get(key) != expected}
    if marker.get("parent_metadata") != parent or mismatches:
        raise RuntimeError(f"dispatch preflight marker invariant failed {variant}: {mismatches}")
    return {
        "pass": True,
        "parent_state": str(parent_path),
        "child_marker": str(marker_path),
        "command": command,
        "parent": parent,
        "child": marker,
        "stdout": proc.stdout,
...
def preflight(repo: Path, output_dir: Path | None = None) -> dict[str, Any]:
    if output_dir is None:
        raise RuntimeError("Fix3 preflight requires an output directory for durable dispatch markers")
    output_dir.mkdir(parents=True, exist_ok=False)
    sources: dict[str, str] = {}
    result: dict[str, Any] = {
        "experiment": "Stretch 031 Fix3 local generated-source and dispatch preflight",
        ...
        "venv_path_regression_guard": assert_no_venv_python_dereference(repo),
        "variants": {},
        "dispatch": {},
    }
    for variant in ("M5", "M2"):
        ...
        result["variants"][variant] = {"facts": facts, "wrapper_phase": phase, "rendered_source": str(rendered)}
    ...
    if normalized_m5 != sources["M2"]:
        raise RuntimeError("Stretch 031 Fix3 single-factor invariant failed: normalized M5 source differs from M2")
    ...
    for variant in ("M5", "M2"):
        result["dispatch"][variant] = dispatch_preflight(repo, variant, output_dir)
    result["classification"] = "STRETCH_031_FIX3_PREFLIGHT_PASS"
...
def default_preflight_dir(repo: Path) -> Path:
    ...
    return repo / "results-local/stretch/single-pass-m2-m5-geometry-comparison-031-fix3/preflight" / stamp


def run_variant(variant: str, launcher_path: Path, argv: list[str] | None = None) -> int:
    """Run a Fix3 variant with its explicit executable shim identity."""
    argv = list(sys.argv[1:] if argv is None else argv)
    repo = Path(__file__).resolve().parents[1]
    launcher_path = launcher_path.resolve()
    expected_launcher = (repo / LAUNCHER_PATHS[variant]).resolve()
    if launcher_path != expected_launcher:
        raise RuntimeError(f"Fix3 explicit launcher mismatch: {launcher_path} != {expected_launcher}")
    if not launcher_path.is_file():
        raise RuntimeError(f"Fix3 launcher missing: {launcher_path}")
    if argv and argv[0] in {"--preflight", "--render-only", "--dispatch-preflight"}:
    ...
    if argv and argv[0] != "--child":
        raise SystemExit(f"usage: {launcher_path.name} [--preflight|--dispatch-preflight|--render-only|--child]")
    wrapper, _, paths, s027, modules = build_wrapper(repo, variant)
    namespace = {
        "__name__": "__main__",
        # This explicit shim path is captured by the inherited parent and is
        # consequently the actual script passed to its canonical MLX child.
