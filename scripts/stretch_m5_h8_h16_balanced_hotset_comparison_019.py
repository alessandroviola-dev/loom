...
Runs the frozen exact M=5 target path with either 8 or 16 persistent transformer
layers in balanced ABBA order. The only intended scientific factor is hotset
residency. No deliberate cache purge is performed.
"""
...
SOURCE_H8_PATH = Path("scripts/stretch_five_token_oracle_block_confirmation_017.py")
SOURCE_H8_BLOB = "6171440736badf5150297f9c8945209fe49d0826"
...
def aggregate_variant(records: list[dict]) -> dict:
    ...
    min_free_values = [int(r["min_memory_free_percent"]) for r in records if r.get("min_memory_free_percent") is not None]
    peak_swap_values = [float(r["peak_swap_used_mb"]) for r in records if r.get("peak_swap_used_mb") is not None]
...
def main() -> int:
    ...
    h8_path = repo / SOURCE_H8_PATH
    h16_path = repo / SOURCE_H16_PATH
    observed_h8 = git_blob(h8_path, repo)
    observed_h16 = git_blob(h16_path, repo)

    print("LOOM Stretch 019 — Balanced M5 H8 vs H16 Hotset Comparison")
    print(f"Stretch 017 H8 blob: {observed_h8}")
    print(f"Stretch 019 H16 helper blob: {observed_h16}")
    if observed_h8 != SOURCE_H8_BLOB or observed_h16 != SOURCE_H16_BLOB:
        print("Source provenance: FAIL", file=sys.stderr)
        return 2
    print("Source provenance: PASS")
    print("Balanced run order: H8 -> H16 -> H16 -> H8")
    print("Frozen exact oracle block size: M=5")
    print("Scientific factor: persistent transformer layers 8 -> 16 ONLY")
    ...
    root = repo / "results-local" / "stretch" / "m5-h8-h16-balanced-hotset-comparison-019"
    ...
    summary: dict = {
        "experiment": "Stretch 019 — Balanced M5 H8 vs H16 Hotset Comparison",
        "classification": "RUNNING",
        "run_id": run_id,
        "source_provenance": {
            "h8_path": str(SOURCE_H8_PATH),
            "h8_expected_blob": SOURCE_H8_BLOB,
            "h8_observed_blob": observed_h8,
        ...
        "run_order": RUN_ORDER,
        "oracle_block_size": 5,
        "scientific_factor": "persistent transformer hotset H8 -> H16 only",
    ...
    save()
    scripts = {"H8": h8_path, "H16": h16_path}
    ...
    h8_records = [r for r in summary["attempts"] if r["variant"] == "H8"]
    h16_records = [r for r in summary["attempts"] if r["variant"] == "H16"]
    h8 = aggregate_variant(h8_records)
    h16 = aggregate_variant(h16_records)

    comparison = {
        "h8": h8,
        "h16": h16,
        "h16_over_h8_pooled_target_rate_ratio": safe_ratio(
            h16["pooled_target_verification_tokens_per_second"],
            h8["pooled_target_verification_tokens_per_second"],
        ),
        "h16_over_h8_median_block_wall_ratio": safe_ratio(
            h16["median_block_full_pass_seconds"], h8["median_block_full_pass_seconds"]
        ),
        "h16_over_h8_median_materialize_ratio": safe_ratio(
            h16["median_block_materialize_seconds"], h8["median_block_materialize_seconds"]
        ),
        "h16_over_h8_median_forward_ratio": safe_ratio(
            h16["median_block_forward_seconds"], h8["median_block_forward_seconds"]
        ),
        "h16_over_h8_mean_full_pass_read_bytes_per_block_ratio": safe_ratio(
            h16["mean_full_pass_disk_read_bytes_per_block"], h8["mean_full_pass_disk_read_bytes_per_block"]
        ),
        "h16_over_h8_mean_materialize_read_bytes_per_block_ratio": safe_ratio(
            h16["mean_materialize_disk_read_bytes_per_block"], h8["mean_materialize_disk_read_bytes_per_block"]
        ),
        "h16_over_h8_hotset_bytes_ratio": safe_ratio(
            h16["persistent_hotset_bytes"], h8["persistent_hotset_bytes"]
        ),
    }
    comparison["observed_higher_pooled_target_rate"] = (
        "H16"
        if h16["pooled_target_verification_tokens_per_second"] > h8["pooled_target_verification_tokens_per_second"]
        else "H8"
    )

    summary["comparison"] = comparison
    summary["classification"] = "M5_H8_H16_BALANCED_HOTSET_COMPARISON_PASS"
    summary["disk_free_after_gib"] = gib_free(repo)
    save()

    print("Classification: M5_H8_H16_BALANCED_HOTSET_COMPARISON_PASS")
    print(f"H8 pooled target-verification tok/s: {h8['pooled_target_verification_tokens_per_second']}")
    print(f"H16 pooled target-verification tok/s: {h16['pooled_target_verification_tokens_per_second']}")
    print(f"H16/H8 target-rate ratio: {comparison['h16_over_h8_pooled_target_rate_ratio']}")
    print(f"H8 median block wall: {h8['median_block_full_pass_seconds']} s")
    print(f"H16 median block wall: {h16['median_block_full_pass_seconds']} s")
    print(f"H16/H8 median materialize ratio: {comparison['h16_over_h8_median_materialize_ratio']}")
    print(f"H16/H8 median forward ratio: {comparison['h16_over_h8_median_forward_ratio']}")
    print(f"H16/H8 mean full-pass process-read bytes/block ratio: {comparison['h16_over_h8_mean_full_pass_read_bytes_per_block_ratio']}")
    print(f"H8 persistent hotset bytes: {h8['persistent_hotset_bytes']}")
    print(f"H16 persistent hotset bytes: {h16['persistent_hotset_bytes']}")
    print(f"H8 minimum observed free memory: {h8['minimum_observed_free_memory_percent']}%")
