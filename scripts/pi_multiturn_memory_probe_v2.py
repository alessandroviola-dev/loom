#!/usr/bin/env python3
"""LOOM Pi Multi-turn Memory Probe 002.

Uses an explicit run-local Pi extension tool (`probe_step`) to force controlled
model -> tool -> result -> model round trips. Built-in tools are disabled.
Each valid tool call returns an opaque token required by the next call, so the
model cannot know a later valid token before receiving the preceding result.
    ...
    "providers": {
        "ollama": {
            ...
            "models": [
                {
                    "id": MODEL_ID,
                    "name": "Qwen 3.5 4B MLX (Ollama / LOOM controlled multiturn probe)",
                    "reasoning": False,
                    "input": ["text"],
                    "contextWindow": 4096,
                    "maxTokens": 2048,
...
def summarize_events(events: list[dict]) -> dict:
    ...
    for event in events:
        ...
        if typ == "tool_execution_start":
            ...
            ends.append({"tool": event.get("toolName"), "is_error": bool(event.get("isError")), "details": details})
...
def run_condition(*, label: str, depth: int, repo_root: Path, workspace: Path,
    ...
    env.update({
        ...
        "LOOM_PROBE_INITIAL_TOKEN": initial_token,
    })

    before = memory_snapshot(repo_root)
    prompt = (
        "LOOM Pi Multi-turn Memory Probe 002. Use ONLY the probe_step tool. "
        f"The initial token is: {initial_token}. "
        "Call probe_step with exactly that token. If the result contains NEXT_TOKEN, "
        "call probe_step again using exactly that new token. Continue one step at a time. "
        "Never guess or reuse a token. When the tool result contains STOP, make no more tool calls "
        "and reply exactly DONE with no explanation."
    )
    command = [
    ...
    invalid = [x for x in ends if x.get("details", {}).get("advanced") is False]
    ...
    success = (
        ...
        and len(advanced) == depth and not invalid and not other_tools
        and all(x.get("is_error") is False for x in ends)
        and (ends[-1].get("details", {}).get("stop") is True if ends else False)
    )

    return {
        "label": label, "target_depth": depth, "success": success,
        "exit_code": exit_code, "timed_out": timed_out, "wall_seconds": wall,
        "tool_call_count": len(starts), "advanced_count": len(advanced),
        "invalid_count": len(invalid), "other_tools": other_tools,
        "final_text": s["final_text"], "event_errors": s["event_errors"],
        "jsonl_parse_errors": parse_errors, "last_nonzero_usage": s["last_nonzero_usage"],
...
def print_record(r: dict) -> None:
    ...
    print(
        ...
        f"calls={r['tool_call_count']} advanced={r['advanced_count']} invalid={r['invalid_count']} "
...
def main() -> int:
    ...
    run_dir = repo / "results-local" / "pi-multiturn-memory-v2" / run_id
    raw = run_dir / "raw"
    ...
    summary = {
        ...
        "experiment": "Pi Multi-turn Memory Probe 002", "model": MODEL_ID,
        "context": 4096, "conditions": [], "initial_memory": memory_snapshot(repo),
    }

    print("LOOM Pi Multi-turn Memory Probe 002")
    print("=== COLD DEPTH ARM ===")
    deepest_valid = False
    for label, depth in [("cold-1turn", 1), ("cold-4turn", 4), ("cold-8turn", 8)]:
        run(["ollama", "stop", MODEL_ID], repo)
        rec = run_condition(label=label, depth=depth, repo_root=repo, workspace=workspace,
                            pi_agent_dir=agent_dir, extension_path=extension, raw_dir=raw,
    ...
    if deepest_valid:
        rec = run_condition(label="warm-1turn-after-8", depth=1, repo_root=repo, workspace=workspace,
                            pi_agent_dir=agent_dir, extension_path=extension, raw_dir=raw,
                            timeout=args.task_timeout)
        summary["conditions"].append(rec)
        print_record(rec)
