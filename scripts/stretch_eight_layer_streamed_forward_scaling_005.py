...
quantization, parity gate, residency gates and safety boundaries.

No full Qwen model construction, tokenizer, embedding lookup, KV cache,
LM head or token generation.
"""
...
SOURCE_004_BLOB = "426423c9d9b7bd7bd1c6a3620197ad5212c678e6"
...
for layer_id in layers:
    ...
    resident_weights.append(selected)
...
save(
    ...
    block_count=len(resident_blocks),
)

started = time.perf_counter()
mx.eval({f"layer_{layer_id}": block.parameters() for layer_id, block in zip(layers, resident_blocks)})
...
started = time.perf_counter()
resident_out = resident_input
for block in resident_blocks:
    resident_out = block(resident_out, resident_mask, None)
...
# Reset peak accounting for the streamed section while preserving resident_out.
mx.reset_peak_memory()

# Streamed path: one real transformer block at a time.
stream_input = make_input()
stream_mask = create_attention_mask(stream_input, None)
stream_cycles = []
total_stream_materialize_wall = 0.0
total_stream_forward_wall = 0.0
...
for layer_id in layers:
    ...
    total_stream_materialize_wall += materialize_wall
    ...
    total_stream_forward_wall += forward_wall
...
max_stream_materialized_delta = max(c["materialized_delta_bytes"] for c in stream_cycles)
resident_materialized_delta = resident_post_materialize_active - resident_base_active
residency_ratio = (
    resident_materialized_delta / max_stream_materialized_delta
    if max_stream_materialized_delta > 0
    else None
)
...
final = {
    ...
    "stream": {
        "max_materialized_delta_bytes": max_stream_materialized_delta,
        "total_materialize_wall_seconds": round(total_stream_materialize_wall, 6),
        "total_forward_wall_seconds": round(total_stream_forward_wall, 6),
        "peak_bytes": stream_peak,
        "resident_to_streamed_materialized_ratio": residency_ratio,
    },
    "parity": {
...
def main() -> int:
    ...
    source004 = repo / "scripts" / "stretch_two_layer_streamed_micro_forward_parity_004.py"
    source002 = repo / "scripts" / "stretch_single_layer_mlx_materialization_002.py"
    observed004 = git_blob(source004, repo)
    observed002 = git_blob(source002, repo)

    print("LOOM Stretch 005 — Eight-Layer Streamed Forward Scaling")
    print(f"Stretch 004 source blob: {observed004}")
    print(f"Stretch 002 helper blob: {observed002}")
    if observed004 != SOURCE_004_BLOB:
        print(f"Stretch 004 provenance: FAIL expected {SOURCE_004_BLOB}", file=sys.stderr)
        return 2
    if observed002 != SOURCE_002_BLOB:
        print(f"Stretch 002 provenance: FAIL expected {SOURCE_002_BLOB}", file=sys.stderr)
        return 2
    ...
    run_dir = repo / "results-local" / "stretch" / "eight-layer-streamed-forward-scaling-005" / run_id
    ...
    def finish(code: int) -> int:
        ...
        if summary.get("resident"):
            print(f"Resident eight-layer materialized delta: {summary['resident'].get('materialized_delta_bytes')} B")
        if summary.get("stream"):
            s = summary["stream"]
            print(f"Max streamed one-layer materialized delta: {s.get('max_materialized_delta_bytes')} B")
            print(f"Resident/streamed materialized ratio: {s.get('resident_to_streamed_materialized_ratio')}")
            print(f"Stream total materialize wall: {s.get('total_materialize_wall_seconds')} s")
            print(f"Stream total forward wall: {s.get('total_forward_wall_seconds')} s")
    ...
    print("Tokenizer/embedding/KV/LM-head/token generation: NONE")
    print(f"Resident control layers: {LAYERS}")
    print(f"Streamed path: {' -> evict -> '.join(map(str, LAYERS))} -> evict")
    ...
    if not source004.is_file() or not source002.is_file() or not venv_py.is_file() or not weight.is_file() or not config_path.is_file():
        summary["classification"] = "PREFLIGHT_FAIL"
    ...
    expected_resident = len(LAYERS) * EXPECTED_LAYER_BYTES
    ...
    for cycle in child["stream_cycles"]:
        ...
        if abs(cycle["materialized_delta_bytes"] - EXPECTED_LAYER_BYTES) > 1 * 1024 * 1024:
            ...
            summary["failure_reason"] = (
                f"layer {cycle['layer_id']} materialized delta {cycle['materialized_delta_bytes']} B"
            )
            return finish(7)
    ...
    if not child["parity"]["pass"]:
        ...
        summary["failure_reason"] = (
            f"max abs diff {child['parity']['max_abs_diff']} > threshold {child['parity']['threshold']}"
        )
        return finish(8)
