...
Fix2 added explicit shim-path propagation and a durable no-model dispatch sentinel. Source rendering/compilation and the normalized M5/M2 source comparison passed, but M5 dispatch failed before child-marker creation. `dispatch_preflight()` called `.resolve()` on the canonical venv `bin/python`; macOS dereferenced it to framework Python, where `mlx` metadata is absent. The preserved parent state records that wrong executable and no model was loaded. This remains harness-only, scientific result **NONE**; no ABBA or retry followed. See `research/stretch/single-pass-m2-m5-geometry-comparison-031-fix2-amendment.md`.
